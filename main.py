...
# PAGE CONFIG
st.set_page_config(
    page_title="Dcarbonize stream Activity",
    page_icon="🏗️"
)
# --------------------------
...
# Page Header
with header:
    st.title("Dcarbonize App📈")
# About info
with header.expander("About this app🔽", expanded=True):
    st.markdown(
        """ This is dashboard for Dcarbonize web viewing
        """
    )
# --------------------------
...
with input:
    ...
    speckleToken = tokenCol.text_input("Speckle token", "7468a4ccbce134920c46d0ecfd84979263c4e10a78")  # Removed trailing comma
    # -------
...
with graphs:
    ...
    # create graph
    branch_count_graph = px.bar(branch_counts, x=branch_counts.branchName, y=branch_counts.totalCommits,
                                color=branch_counts.branchName, labels={"branchName": "", "totalCommits": ""})
    # update layout
